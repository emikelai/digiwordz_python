...
phone_number = input('Enter your phone number (no dashes): ')
phone_number_list = list(phone_number) #split each number into an array

i = 0           #initialize counter that keeps track of number of matches
match_list = [] #initialize list of matches to phone number length
match_list2 = [] #initialize list of matches that contains final list of matches

for line in dictionary_list:    #this for loop will exhaustively iterate through the dictionary list
    #print(line, end='')
    ...
    if len(phone_number_list) == len(line) - 1: #check if phone number length matches word length
        ...
        #    print(line_list[i])
        match_list.append(line) #add matching length word in dictionary to the match_list list
...
for letter in range(len(phone_number_list)): #only execute the contents of for loop X times where X is the length of the phone number
    ...
    #print_list(num2letterlist)
    for line in match_list: #go through each word in match_list (match_list contains on word that have the same length as phone number)
        ...
        if line[letter] in num2letterlist:  # if digit of phone number matches corresponding position of word in dictionary
